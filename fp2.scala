...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def f(n:String, s:String): String = n + s + term
    xs match {
      case Nil => ""
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
    def f(n:String, s:String) : String = delimiter + n + s 
    xs match {
      case Nil    => ""
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    firstIndexNumGreaterThanAux(0,a,xs)
  }
  
  def firstIndexNumGreaterThanAux (indx : Int, a : Int, xs : List[Int]) : Int = {
    xs match {
      case Nil => throw new RuntimeException()
      case y::ys => {
        if(y>=a) {
          ...
          firstIndexNumGreaterThanAux(indx+1,a,ys)
        }
      } 
    }
  }
